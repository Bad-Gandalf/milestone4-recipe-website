...
"""These test should be run when the app is using mySQL as the database. 
...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_recipe_description(self): 
        ...
        tester = app.test_client(self)
        response = tester.get('/recipe/28', content_type='html/text')
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_by_allergen(self): 
        ...
        tester = app.test_client(self)
        response = tester.post('/find_recipe_allergen_name', data=dict(allergens="14"), follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(b"Lorraine Pascale" in response.data) 
        self.assertTrue(b"Jamaican Rum Truffles" in response.data)
    ...
    def test_get_cuisines(self):
        # Test Page for cuisine list retrieval loads
        tester = app.test_client(self)
    ...
    def test_edit_recipe(self):
        ...
        tester = app.test_client(self)
        response = tester.get('/edit_recipe/{}'.format(28), content_type='html/text')
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_cuisine(self):
        ...
        tester = app.test_client(self)
        response = tester.get('/edit_cuisine/{}'.format(2), content_type='html/text')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(b"Szechuan cuisine is a type of Chinese cuisine" in response.data)
    ...
    def test_edit_country(self):
        ...
        tester = app.test_client(self)
        response = tester.get('/edit_country/{}'.format(2), content_type='html/text')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(b"France" in response.data)
