...
def get_recipes():
    ...
    pagination = Pagination(page=page, total=_recipes.count(),
                            record_name='recipes')
...
def search_recipes():
    ...
    return render_template("searchrecipe.html", allergens=allergen_list,
...
def find_recipe_by_name():
    page = get_page()
    search_term = {"recipe_name": {'$regex': request.form['recipe_name'],
                                   '$options': 'i'}}
    _recipes = mongo.db.recipes.find(search_term).sort('upvotes',
                                                       pymongo.DESCENDING)
    pagination = Pagination(page=page, total=_recipes.count(),
                            record_name='recipes')
...
def find_recipe_by_ingredient():
    page = get_page()
    search_term = {"ingredients": {'$regex': request.form['ingredient_name'],
                                   '$options': 'i'}}
    _recipes = mongo.db.recipes.find(search_term).sort('upvotes',
                                                       pymongo.DESCENDING)
    pagination = Pagination(page=page, total=_recipes.count(),
...
def add_recipe():
    ...
    _allergens = mongo.db.allergens.find().sort("allergen_name",
                                                pymongo.ASCENDING)
    _countries = mongo.db.countries.find().sort("country_name",
                                                pymongo.ASCENDING)
    return render_template("addrecipe.html", allergens=_allergens,
                           cuisines=_cuisines, countries=_countries)
...
# Different templates required to send id for mysql many to many searches
# and updates
@app.route('/edit_recipe/<recipe_id>')
def edit_recipe(recipe_id):
    the_recipe = mongo.db.recipes.find_one({"_id": ObjectId(recipe_id)})
    cuisines = mongo.db.cuisines.find().sort("cuisine_name",
                                             pymongo.ASCENDING)
    allergens = mongo.db.allergens.find().sort("allergen_name",
                                               pymongo.ASCENDING)
    _countries = mongo.db.countries.find().sort("country_name",
                                                pymongo.ASCENDING)
...
# Deleting in mysql requires deletion of many to many table rows that contain
...
def get_cuisines():
    ...
    pagination = Pagination(page=page, total=_cuisines.count(),
                            record_name='cuisines')
...
def get_countries():
    ...
    _countries = mongo.db.countries.find().sort("country_name",
                                                pymongo.ASCENDING)
    pagination = Pagination(page=page, total=_countries.count(),
...
def get_allergens():
    page = get_page()
    _allergens = mongo.db.allergens.find().sort("allergen_name")
    pagination = Pagination(page=page, total=_allergens.count(),
                            record_name='allergens')
...
# Increase the upvotes by clicking on the thumbs up button, then be redirected
# to that specific recipe page
...
def display_stats():
    cursor = mongo.db.recipes.find({}, {'_id': 0, "username": 1,
                                        "recipe_name": 1, "author": 1,
                                        "prep_time": 1, "cook_time": 1,
                                        "upvotes": 1, "cuisine_name": 1,
                                        "country": 1})
